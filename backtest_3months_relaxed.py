"""
Backtesting multi-par (3 meses RELAJADO): 27-Mar-2026 al 27-May-2026.
Parametros de riesgo relajados:
- Limite diario: 5% del capital
- Rachas: reducir al 50% con 4 perdidas, kill switch con 7
- Drawdown: igual (10% -> 50% riesgo, 15% -> 25% riesgo)
Incluye parche de sesion siempre activa.
"""
import sys, os, time
sys.path.insert(0, '.')
import numpy as np
...
# ========== GESTION DINAMICA DE RIESGO (RELAJADA) ==========
...
def backtest_pair(symbol, capital, risk_manager, peak_capital):
    ...
    # PARCHE DE SESION SIEMPRE ACTIVA
...
def main():
    ...
    if all_trades:
        ...
        csv_name = 'backtest_3months_relaxed.csv'
        ...
        print(f"Maximo drawdown (capital): ${max_dd:,.2f}")
        print(f"Capital final: ${capital:,.2f}")
        print(f"Peak capital: ${peak_capital:,.2f}")
    else:
        print("\nNo se generaron trades en ningun par.")
