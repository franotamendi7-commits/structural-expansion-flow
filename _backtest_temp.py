...
"""
BACKTEST INSTITUCIONAL V1 – Q4 2025 (1 oct – 31 dic 2025)
- Fase 5 con 5 pares (BTC, ETH, SOL, XRP, BNB)
- Features enriquecidas (15+ variables)
- Slippage ATR (0.1 * ATR 5m) en entrada y stops
- Checklist de auditoría automático
"""
...
parser.add_argument('--phase_mode', type=str, default='strict', choices=['strict','flexible','free'])
parser.add_argument('--output', type=str, default='backtest_institucional.csv')
...
SLIPPAGE_ATR_MULT = 0.1     # multiplicador de ATR 5m para slippage
...
class ScalpingEngine:
    ...
    def run(self, klines, current_price, current_ts=None):
        try:
            ...
            # Body ratio de la última vela 4h
            k4h = klines['4h']
...
def simulate_exit(direction, entry_real, sl_original, tp1, contracts_total, candles, start_idx, atr_5m):
    slippage_exit = SLIPPAGE_ATR_MULT * atr_5m  # empeora la salida en stops
    ...
    for j in range(start_idx+1, len(candles)):
        ...
        if direction == 'bullish':
            ...
            if partial_done and high >= tp2:
                ...
                remaining = 0; break
            if low <= sl_active:
                exit_price = sl_active - slippage_exit   # <-- slippage en contra
                if not partial_done:
...
for sym in SYMBOLS:
    ...
    for i in range(10, len(candles_1h)):
        ...
        if res['setup_state'] == 'EXECUTE' and res['trade'] is not None:
            ...
            # Calcular ATR 5m hasta el momento de la entrada (para slippage)
            ...
            # Construir registro con features
            trade_record = {
                'symbol': sym,
            ...
            if 'features' in res:
                trade_record.update(res['features'])
            all_trades.append(trade_record)
            stats[exit_type] += 1

# ═══════════════════ FUNCIÓN DEL CHECKLIST (NUEVA) ═══════════════════
def generar_checklist(nombre_archivo, df, stats, capital_final, max_dd, max_dd_pct, total_pnl, win_rate):
    from datetime import datetime, timezone
    with open(nombre_archivo, 'w', encoding='utf-8') as f:
        f.write("# Auditoría Backtest Institucional V1\n")
        f.write(f"- **Fecha de ejecución:** {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M UTC')}\n")
        f.write(f"- **Período testeado:** {START_DATE.strftime('%Y-%m-%d')} → {END_DATE.strftime('%Y-%m-%d')}\n")
        f.write(f"- **Pares:** {', '.join(SYMBOLS)}\n")
        f.write(f"- **Capital inicial:** ${INITIAL_CAPITAL:.2f}\n")
        f.write(f"- **Riesgo fijo por operación:** {FIXED_RISK_PCT*100:.1f}%\n")
        f.write(f"- **Costos modelados:**\n")
        f.write(f"  - Comisión taker: {COMMISSION*100:.3f}%\n")
        f.write(f"  - Spread fijo: {SPREAD*100:.3f}%\n")
        f.write(f"  - Slippage: {SLIPPAGE_ATR_MULT} × ATR(5m, 14) en entrada y stops\n")
        f.write(f"- **Look-ahead:** Se usa precio de cierre de vela 1h (mejorable a precio de apertura siguiente)\n")
        f.write(f"- **Gestión de salida:** TP1 parcial 60%, breakeven al 50% del TP1, TP2 restante\n")
        f.write("\n## Resultados\n")
        f.write(f"- **Trades totales:** {len(df)}\n")
        f.write(f"- **Win Rate:** {win_rate:.1f}%\n")
        f.write(f"- **PnL neto total:** ${total_pnl:,.2f}\n")
        f.write(f"- **Drawdown máximo:** ${max_dd:,.2f} ({max_dd_pct:.1f}% del pico)\n")
        f.write(f"- **Capital final:** ${capital_final:,.2f}\n")
        # Profit Factor
        profit_sum = df[df['pnl_neto']>0]['pnl_neto'].sum()
        loss_sum = abs(df[df['pnl_neto']<=0]['pnl_neto'].sum())
        pf = profit_sum / loss_sum if loss_sum != 0 else float('inf')
        f.write(f"- **Profit Factor:** {pf:.2f}\n")
        f.write("\n### Desglose de salidas\n")
        f.write(f"- TP2 alcanzado: {stats['tp2_reached']}\n")
        f.write(f"- TP1 parcial + SL: {stats['tp1_partial_then_sl']}\n")
        f.write(f"- SL completo: {stats['full_sl']}\n")
        f.write("\n### Por par\n")
        for sym in SYMBOLS:
            ...
            if len(sym_df) > 0:
                ...
                f.write(f"- {sym}: {len(sym_df)} trades | WR {sym_win:.1f}% | PnL ${sym_pnl:,.2f}\n")
        f.write("\n## Features registradas\n")
        features_cols = [c for c in df.columns if c not in ['symbol','entry_time','direction','entry_signal','entry_real','sl_original','tp1','contracts','pnl_neto','exit_events','exit_type','explanation','score']]
        f.write(f"- {', '.join(features_cols)}\n")
        f.write("\n## Conclusión preliminar\n")
        f.write("_(Completar después del análisis out‑of‑sample)_\n")
    print(f"📋 Checklist guardado en {nombre_archivo}")

# ═══════════════════ RESULTADOS Y CHECKLIST ═══════════════════
if all_trades:
    ...
    print("\n========== RESULTADOS Q4 2025 (FASE 5 – 5 PARES) ==========")
    ...
    print(f"\nFeatures almacenadas: {[c for c in df.columns if c not in ['symbol','entry_time','direction','entry_signal','entry_real','sl_original','tp1','contracts','pnl_neto','exit_events','exit_type','explanation','score']]}")

    # ─── LLAMADA AL CHECKLIST (NUEVA) ───
    generar_checklist(OUTPUT_CSV.replace('.csv','.md'), df, stats, capital, max_dd, max_dd_pct, total_pnl, win_rate)
else:
