"""
BACKTEST 3 MESES (17-Mar → 17-Jun-2026) con el motor exacto que pegaste,
más reglas de salida reales (parcial 60% TP1, breakeven 50%, TP2).
"""
...
START_DATE = datetime(2026, 3, 17, 0, 0, tzinfo=timezone.utc)
...
# ──────────── MOTOR ORIGINAL (copiado exacto) ────────────
SYMBOL_CONFIG = {
    'BTCUSDT': {'supertrend_multiplier':3.0,'choppiness_neutral_threshold':70.0,'fibonacci_days':30,'tp_ratio':1.5},
    'ETHUSDT': {'supertrend_multiplier':2.8,'choppiness_neutral_threshold':69.0,'fibonacci_days':30,'tp_ratio':1.8},
    'SOLUSDT': {'supertrend_multiplier':2.5,'choppiness_neutral_threshold':68.0,'fibonacci_days':90,'tp_ratio':2.0},
    'XRPUSDT': {'supertrend_multiplier':2.5,'choppiness_neutral_threshold':65.0,'fibonacci_days':60,'tp_ratio':1.8},
}
...
class SessionFilter:
    @staticmethod
    def is_trading_session(): return True  # parche para backtest
...
class ScalpingEngine:
    ...
    def run(self, klines, current_price):
        # klines: dict con todos los timeframes truncados
        try:
            ...
            if signal in ('LONG', 'SHORT'):
                ...
                if enhanced['veto']:
                    ...
                    signal = 'WAIT'; setup_state = 'INVALID'
            if signal == 'LONG' and enhanced['st'].get('bias', '') != 'bullish':
                if self.debug_filters: print("[DEBUG] VETO LONG: ST no alineado")
                signal = 'WAIT'; setup_state = 'INVALID'
...
for sym in SYMBOLS:
    ...
    for i in range(10, len(candles_1h)):
        ...
        if res['setup_state'] == 'EXECUTE' and res['trade'] is not None:
            ...
            # Índice en velas 5m para simular salida
...
if all_trades:
    ...
    csv_name = 'backtest_3m_MOTOR_REAL.csv'
