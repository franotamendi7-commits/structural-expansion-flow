"""
Backtesting multi-par (3 meses ULTRA RELAJADO): 27-Mar-2026 al 27-May-2026.
Parametros de riesgo ultra relajados para prueba.
"""
import sys, os, time
sys.path.insert(0, '.')
import numpy as np
...
START_DATE = datetime(2026, 3, 27, tzinfo=timezone.utc)
END_DATE   = datetime(2026, 5, 27, tzinfo=timezone.utc)
MAX_KLINES = 1000
BASE_URL   = "https://api.binance.com/api/v3/klines"
INITIAL_CAPITAL = 100.0
BASE_RISK_PCT = 0.01
MAX_DAILY_LOSS_PCT = 0.10          # ← 10% (casi sin límite diario)
MAX_CONSECUTIVE_LOSSES = 9         # ← kill switch muy lejano

# ========== GESTIÓN DINÁMICA DE RIESGO (ULTRA RELAJADA) ==========
class DynamicRiskManager:
    def __init__(self, base_risk_pct=0.01, max_daily_loss_pct=0.10, max_consecutive_losses=9):
    ...
    def get_adjusted_risk(self, capital, peak_capital):
        ...
        # Solo activa protección con drawdown MUY grande
        if drawdown > 0.25:
            factor_dd = 0.5
        elif drawdown > 0.20:
            factor_dd = 0.75
        ...
        # Rachas: kill switch muy lejano
        if self.consecutive_losses >= self.max_consecutive_losses:
            return 0.0
        elif self.consecutive_losses >= 7:   # reducción solo con 7 pérdidas seguidas
            factor_loss = 0.5
        else:
            factor_loss = 1.0

        # Límite diario muy laxo
...
def fetch_klines_range(symbol, interval, start_dt, end_dt):
    ...
    while start_ms < end_ms and page < max_pages:
        params = {'symbol': symbol, 'interval': interval, 'limit': MAX_KLINES, 'startTime': start_ms, 'endTime': end_ms}
        ...
        data = resp.json()
        if not data: break
        batch = [{'timestamp': int(k[0]), 'open': float(k[1]), 'high': float(k[2]), 'low': float(k[3]), 'close': float(k[4]), 'volume': float(k[5])} for k in data]
...
def backtest_pair(symbol, capital, risk_manager, peak_capital):
    ...
    for i in range(100, len(candles)):
        ...
        if res['setup_state'] == 'EXECUTE' and res.get('trade'):
            ...
            entry = trade['entry']; sl = trade['sl']; tp1 = trade['tp1']; contracts = trade.get('contracts', 0)
            direction = res.get('direction', 'neutral')
            if entry == 0 or contracts == 0: continue
            exit_price = None; exit_reason = None
            for j in range(i+1, len(candles)):
                high = candles[j]['high']; low = candles[j]['low']
                if direction == 'bullish':
                    if low <= sl: exit_price = sl; exit_reason = 'stop_loss'; break
                    if high >= tp1: exit_price = tp1; exit_reason = 'take_profit'; break
                else:
                    if high >= sl: exit_price = sl; exit_reason = 'stop_loss'; break
                    if low <= tp1: exit_price = tp1; exit_reason = 'take_profit'; break
            if exit_price is not None:
                ...
                trades.append({'symbol': symbol, 'entry_time': datetime.fromtimestamp(candles[i]['timestamp']/1000, tz=timezone.utc).isoformat(), 'exit_time': exit_dt.isoformat(), 'signal': direction, 'entry': entry, 'exit': exit_price, 'pnl': pnl, 'reason': exit_reason, 'risk_pct_used': dyn_risk_pct, 'contracts': contracts, 'consecutive_losses': risk_manager.consecutive_losses, 'daily_pnl': risk_manager.daily_pnl})
...
def main():
    ...
    if all_trades:
        ...
        df.to_csv('backtest_3months_ultra_relaxed.csv', index=False)
        win_rate = len(df[df['pnl'] > 0]) / len(df) * 100
        total_pnl = df['pnl'].sum()
        df['capital_curve'] = INITIAL_CAPITAL + df['pnl'].cumsum()
        max_dd = (df['capital_curve'].cummax() - df['capital_curve']).max()
        print(f"📁 {len(df)} trades | Win rate: {win_rate:.1f}% | PnL total: ${total_pnl:,.2f} | Drawdown: ${max_dd:,.2f} | Capital final: ${capital:,.2f}")
    else:
