#!/usr/bin/env python3
"""
Backtest 6 meses (Ene–Jun 2026) – Motor Fase 5 + ML Agent + Slippage/Comisiones
Usa ml_filter.py y ml_model.pkl del proyecto.
"""
...
def load_cached_data():
    ...
    print("Descargando datos (puede tardar unos minutos)...")
    for sym in SYMBOLS:
        print(f" {sym}:")
        DATA_CACHE[sym] = {}
        for tf in ['1d','4h','1h','15m','5m']:
            print(f"   {tf}...", end='', flush=True)
            k = fetch_klines_range(sym, tf, START_DATE - timedelta(days=120), END_DATE)
            DATA_CACHE[sym][tf] = k
            print(f" {len(k)} velas")
    print("Datos listos.")
    return DATA_CACHE

# ═══════════════════ CLASES DEL MOTOR (Fase 5, copia exacta) ═══════════════════
SYMBOL_CONFIG = {
...
class ScalpingEngine:
    ...
    def _empty_result(self, explanation=''):
        return {'signal':'WAIT','setup_state':'INVALID','explanation':explanation,
                'direction':'neutral','trade':None,'score':0,'enhanced':{},'phase_h1':'',
                'vol_ratio':0,'ci':{},'wr':{},'st':{},'mom':{}}
...
for sym in SYMBOLS:
    data = DATA_CACHE[sym]
    candles_1h = [k for k in data['1h'] if k['timestamp'] >= int((START_DATE - timedelta(days=120)).timestamp()*1000)
                  and k['timestamp'] <= int(END_DATE.timestamp()*1000)]
    ...
    for i, c in enumerate(candles_1h):
        if c['timestamp'] >= int(START_DATE.timestamp()*1000):
            start_idx = i; break
    if start_idx < 10: start_idx = 10
    ...
    for i in range(start_idx, len(candles_1h)):
        ...
        if res['setup_state'] != 'EXECUTE' or res['trade'] is None:
            continue

        # ─── ML AGENT ───
        try:
            fib_block = WeeklyAdaptiveFibonacci([k for k in data['1d'] if k['timestamp'] <= ts], days=engine.config['fibonacci_days']).get_current_block(current_price)
            features = {
                'fib_low_key': fib_block[0] if fib_block else 0.0,
                'fib_high_key': fib_block[2] if fib_block else 0.0,
                'fib_width': (fib_block[3]-fib_block[1])/fib_block[1] if fib_block and fib_block[1] else 0.0,
                'ci_value': res['ci'].get('value', 50.0),
                'wr_5m': res['wr'].get('value_5m', -50.0),
                'wr_15m': res['wr'].get('value_15m', -50.0),
                'st_aligned': 1 if res['st'].get('aligned') else 0,
                'st_bias_bullish': 1 if res['st'].get('bias')=='bullish' else 0,
                'st_bias_bearish': 1 if res['st'].get('bias')=='bearish' else 0,
                'mom_score': res['mom'].get('score', 0.0),
                'vol_ratio_5m': res.get('vol_ratio', 1.0),
                'body_ratio_4h': 0.0,
                'hour_of_day': datetime.fromtimestamp(ts/1000, tz=timezone.utc).hour,
                'direction_long': 1 if res['signal'] == 'LONG' else 0,   # ← CORREGIDO
                'phase_compressing': 1 if res['phase_h1']=='compressing' else 0,
                'phase_expanding': 1 if res['phase_h1']=='expanding' else 0,
                'phase_trending': 1 if res['phase_h1']=='trending' else 0,
                'mom_bullish': 1 if res['mom'].get('direction')=='bullish' else 0,
                'mom_bearish': 1 if res['mom'].get('direction')=='bearish' else 0,
                'mom_neutral': 1 if res['mom'].get('direction')=='neutral' else 0
            }
        ...
        stats[exit_type] = stats.get(exit_type, 0) + 1

# ═══════════════════ REPORTE ═══════════════════
if all_trades:
    df = pd.DataFrame(all_trades)
    csv_name = 'backtest_6m_ML.csv'
    df.to_csv(csv_name, index=False)
    print(f"\n📁 {len(df)} trades guardados en {csv_name}")

    total_trades = len(df)
    win_trades = (df['pnl_neto'] > 0).sum()
    loss_trades = (df['pnl_neto'] <= 0).sum()
    gross_profit = df[df['pnl_neto'] > 0]['pnl_neto'].sum()
    gross_loss = abs(df[df['pnl_neto'] <= 0]['pnl_neto'].sum())
    pf = gross_profit / gross_loss if gross_loss > 0 else float('inf')
    win_rate = (win_trades / total_trades * 100) if total_trades else 0
    max_dd = (np.maximum.accumulate(equity) - np.array(equity)).max()
    max_dd_pct = (max_dd / max(equity) * 100) if max(equity) > 0 else 0

    print("\n========== RESULTADOS 6 MESES (MOTOR FASE 5 + ML AGENT) ==========")
    print(f"Período:                 2026-01-01 → 2026-06-22")
    print(f"Capital inicial:         ${INITIAL_CAPITAL:.2f}")
    print(f"Capital final:           ${capital:.2f}")
    print(f"Retorno total:           {((capital/INITIAL_CAPITAL)-1)*100:.1f}%")
    ...
    monthly = df.groupby('month').agg(
        ...
        pnl_neto=('pnl_neto', 'sum')
    )
    for m, row in monthly.iterrows():
        print(f"  {m}: {int(row['trades'])} trades, PnL ${row['pnl_neto']:.2f}")

    # Guardar reporte JSON
    report = {
        'periodo': '2026-01-01 → 2026-06-22',
        'capital_inicial': INITIAL_CAPITAL,
        'capital_final': round(capital, 2),
        'retorno_total_%': round((capital/INITIAL_CAPITAL - 1)*100, 2),
        ...
        'trades_por_mes': monthly['trades'].to_dict()
    }
    with open('backtest_6m_ML.json', 'w') as f:
        json.dump(report, f, indent=2)
    print(f"\n📋 Reporte guardado en backtest_6m_ML.json")
else:
    print("\nNo se generaron trades con ML Agent en el período.")
