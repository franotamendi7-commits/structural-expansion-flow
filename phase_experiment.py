...
def run_backtest(data_all, phase_mode):
    ...
    for sym in SYMBOLS:
        ...
        for i in range(10, len(candles_1h)):
            ...
            atr_5m = 0.001 * current_price  # simplified
    ...
    if not trades_list: return {'trades':0,'wr':0,'pnl':0,'dd':0}
    df = pd.DataFrame(trades_list)
    wr = (df['pnl_neto'] > 0).mean()
    total = df['pnl_neto'].sum()
    curve = (INITIAL_CAPITAL + df['pnl_neto'].cumsum()).values
    peak = np.maximum.accumulate(curve)
    dd = (peak - curve).max()
    return {'trades': len(df), 'wr': wr, 'pnl': total, 'dd': dd}

# ─── MAIN ───
print("🚀 Experimento de Fase de Mercado\n")
print("Descargando datos para los 5 pares...")
data_all = {}
for sym in SYMBOLS:
    data_all[sym] = fetch_historical(sym)

modos = ['strict', 'flexible', 'free']
resultados = {}
for mode in modos:
    print(f"\n🔬 Modo {mode}...")
    res = run_backtest(data_all, mode)
    resultados[mode] = res
    print(f"   Trades: {res['trades']}, WR: {res['wr']:.2%}, PnL: \${res['pnl']:.2f}, DD: \${res['dd']:.2f}")
