    ...
    while True:
        try:
            ...
            for symbol, pos in open_positions.items():
                ...
                if action == 'tp1':
                    close_percent = 0.6; reason = "take_profit"
                elif action == 'tp2':
                    close_percent = 1.0; reason = "take_profit_tp2"
                elif action in ('sl', 'breakeven'):
                    close_percent = 1.0; reason = "stop_loss"
                else:
                    close_percent = 1.0; reason = action
                ...
                if close_side and contracts_to_close > 0:
                    ...
                    real_order = exec_mgr.execute_signal({"symbol": symbol, "side": close_side, "quantity": contracts_to_close})
                    if real_order.get("error"): log(f"Error en orden real: {real_order['error']}")
                    else: log(f"Orden real ejecutada: ID={real_order.get('order_id')}, precio={real_order.get('executed_price')}")
                ...
                if trade_id and entry_time_str:
                    ...
                    except: duration_min = 0.0
                else: duration_min = 0.0
