#!/usr/bin/env python3
"""
Comparación del agente PPO vs reglas fijas en un período OUT‑OF‑SAMPLE (Dic 2025).
"""
import numpy as np
import pandas as pd
import requests
...
# ─── 1. DESCARGA DE DATOS (Diciembre 2025) ──────────────────────
print("📥 Descargando velas de BTCUSDT para Dic 2025...")
...
# ─── 2. DETECCIÓN DE SEÑALES (misma lógica simplificada) ────────
...
print(f"✔ {len(signals)} señales detectadas en Dic 2025.")
if len(signals) == 0:
    print("No se encontraron señales. Probá otro período.")
    exit()

# ─── 3. SIMULACIÓN CON REGLAS FIJAS ────────────────────────────
def simulate_fixed(signal):
    """Devuelve el PnL usando la gestión fija (TP1 60%, BE, etc.)"""
    ...
    for j in range(len(candles)):
        ...
        if direction == 'bullish':
            ...
            if high >= sl_active:
                ...
                total_pnl += pnl - comm
                break
        # Si llegamos al final sin salir, cerramos a mercado
        if j == len(candles) - 1 and remaining > 0:
            if direction == 'bullish':
                pnl = (close - entry) * remaining
            else:
                pnl = (entry - close) * remaining
            comm = close * remaining * commission
            total_pnl += pnl - comm

    return total_pnl

# ─── 4. SIMULACIÓN CON AGENTE PPO ──────────────────────────────
model = PPO.load("rl_exit_model")
print("✅ Modelo PPO cargado.")

def simulate_rl(signal):
    """Devuelve el PnL usando las acciones del agente PPO."""
    env = ExitEnv(
        candles=signal['candles'],
        direction=signal['direction'],
...
for i, sig in enumerate(signals):
    ...
    pnl_fixed.append(pnl_f)
    pnl_rl.append(pnl_r)
    print(f"  Trade {i+1}: Fijo ${pnl_f:.2f} | PPO ${pnl_r:.2f}")

df = pd.DataFrame({'fixed': pnl_fixed, 'rl': pnl_rl})

print("\n" + "="*50)
print("RESULTADOS DE VALIDACIÓN (Dic 2025)")
print("="*50)
...
if df['rl'].sum() > df['fixed'].sum():
    print("✅ El agente PPO SUPERA a las reglas fijas en este período.")
else:
    print("⛔ Las reglas fijas siguen siendo mejores. El agente necesita más entrenamiento.")
