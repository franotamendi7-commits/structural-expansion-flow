"""
Portfolio Backtest - 5 Pairs Simultaneous (Optimized)
=====================================================

Uses cached Parquet data. No API downloads needed.
Runs Institutional Engine V2 on all 5 pairs.
...
def load_cached_data(symbol: str) -> Dict[str, pd.DataFrame]:
    """Load all cached Parquet data for a symbol."""
    files = sorted(CACHE_DIR.glob(f"{symbol}_*.parquet"))
    data = {}
    for f in files:
        parts = f.stem.split("_")
        # e.g. BTCUSDT_15m_20260101_20260730
        interval = parts[1]
...
def main():
    print("=" * 70)
    print(" PORTFOLIO BACKTEST — 5 PAIRS SIMULTANEOUS")
    print(" Institutional Engine V2 | Jan–Jul 2026")
    print("=" * 70)
    ...
    for idx, symbol in enumerate(PAIRS):
        ...
        df_15m = data["15m"]
        df_1h = data.get("1h", data["15m"])  # fallback to 15m if no 1h
        df_4h = data["4h"]
        df_1d = data.get("1d", data["4h"])  # fallback to 4h if no 1d

        print(f"  15m: {len(df_15m)} bars, 1h: {len(df_1h)}, 4h: {len(df_4h)}, 1d: {len(df_1d)}")
    ...
    result = {
        ...
        "period": "2026-01-01 to 2026-07-30",
        "portfolio": {
    ...
    output_file = OUTPUT_DIR / "portfolio_5pairs_results.json"
