"""
Comparativa de parámetros ETHUSDT – 6 meses (1-Dic-2025 → 1-Jun-2026)
Variaciones: supertrend_multiplier, choppiness_neutral_threshold, fibonacci_days
...
# Parámetros originales de ETH
ETH_ORIGINAL_PARAMS = {
    'supertrend_multiplier': 2.8,
    'choppiness_neutral_threshold': 69.0,
    'fibonacci_days': 30,
    'tp_ratio': 1.8
}
...
# ----- Simulador de salidas reales (igual que en backtests anteriores) -----
...
def run_backtest(config_label, param_overrides):
    # Modificar temporalmente la configuración del motor para ETHUSDT
    if hasattr(eng_module, 'SYMBOL_CONFIG'):
        original_cfg = eng_module.SYMBOL_CONFIG.get('ETHUSDT', {}).copy()
        eng_module.SYMBOL_CONFIG['ETHUSDT'] = {**original_cfg, **param_overrides}
    else:
        eng_module.SYMBOL_CONFIG = {'ETHUSDT': param_overrides}
        original_cfg = {}
    ...
    if not candles or len(candles) < 100:
        ...
        if original_cfg:
            eng_module.SYMBOL_CONFIG['ETHUSDT'] = original_cfg
        else:
            del eng_module.SYMBOL_CONFIG['ETHUSDT']
        return None
    ...
    safe_label = config_label.replace(' ','_').replace(':','').replace('=','_')
    csv_name = f"backtest_ETH_{safe_label}.csv"
    if not df.empty:
        df.to_csv(csv_name, index=False)
        print(f"  -> {len(df)} trades guardados en {csv_name}")
...
# ----- MAIN: Ejecutar las 4 configuraciones -----
if __name__ == "__main__":
    variations = {
        "Original": ETH_ORIGINAL_PARAMS,
        "Var A: supertrend=3.2": {**ETH_ORIGINAL_PARAMS, 'supertrend_multiplier': 3.2},
        "Var B: choppiness=72": {**ETH_ORIGINAL_PARAMS, 'choppiness_neutral_threshold': 72.0},
        "Var C: fibonacci_days=45": {**ETH_ORIGINAL_PARAMS, 'fibonacci_days': 45}
    }
    ...
    print("\n" + "="*80)
    print("COMPARATIVA DE PARÁMETROS ETHUSDT (6 meses, condiciones reales)")
    print("="*80)
    ...
    print("-"*80)
    print("Archivos CSV generados con los trades de cada variación.")
