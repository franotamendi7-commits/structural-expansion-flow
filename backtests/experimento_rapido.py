...
print("🚀 Experimento rápido usando caché local\n")
...
def load_cache():
    ...
    for sym in SYMBOLS:
        ...
        print(f"{sym}: 1d={len(sym_data['1d'])} 4h={len(sym_data['4h'])} 1h={len(sym_data['1h'])} 15m={len(sym_data['15m'])} 5m={len(sym_data['5m'])} velas")
    return data_all
...
def run_backtest(data_all, phase_mode):
    ...
    for sym in SYMBOLS:
        ...
        for i in range(10, len(candles_1h)):
            ...
            atr_5m = 0.001 * current_price  # conservador
...
# ──────────────── MAIN ────────────────
print("📂 Cargando datos de backtest_cache/ ...")
data_all = load_cache()
...
for modo in modos:
    ...
    df_trades, _ = run_backtest(data_all, modo)
    if df_trades.empty:
        print(f"   {modo}: sin trades")
        continue
    # Guardar CSV (solo pnl)
    df_trades.to_csv(f'backtest_{modo}.csv', index=False)
    # Métricas sin ML
...
print("\n🧠 Aplicando filtro ML a los CSVs generados...\n")
for modo in ['strict', 'flexible', 'free']:
    ...
    if df.empty:
        print(f"{modo}: sin trades")
        continue
    # Como estos CSVs solo tienen pnl_neto, no podemos aplicar ML
    # Así que este experimento rápido no puede aplicar ML directamente.
    # Mostramos solo los resultados sin ML.
    print(f"{modo} (sin ML): ya fue mostrado arriba.")
    
print("\n✅ Experimento rápido finalizado.")
print("(Para ver el efecto del ML necesitamos los CSVs con features, que este script simplificado no genera.)")
