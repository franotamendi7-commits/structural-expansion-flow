"""
Backtesting multi-par (3 meses): 27-Mar-2026 al 27-May-2026.
RIESGO FIJO 1% constante, sin gestión dinámica.
...
# ========== BACKTEST PARA UN PAR (riesgo fijo) ==========
def backtest_pair_fixed(symbol, capital):
    print(f"\n🔍 Backtesting {symbol}...")
...
def main():
    ...
    for sym in SYMBOLS:
        trades, capital = backtest_pair_fixed(sym, capital)
        all_trades.extend(trades)

    if all_trades:
        df = pd.DataFrame(all_trades)
        csv_name = 'backtest_3months_fixed_risk.csv'
