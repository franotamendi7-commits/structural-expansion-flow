...
print("🚀 Experimento completo CON ML (usa caché local)\n")
...
# ─── INDICADORES Y CLASES DEL MOTOR (COPIADAS DE BACKTEST_INSTITUCIONAL_V1) ───
...
print("\n📊 COMPARATIVA FINAL")
print("Modo      | Sin ML (Trades/WR/PnL/DD)          | Con ML (Trades/WR/PnL/DD)")
for modo in modos:
    s = resultados_sin_ml.get(modo)
    c = resultados_con_ml.get(modo)
    if s and c:
        print(f"{modo:10s} | {s['trades']:4d} {s['wr']:.2%} ${s['pnl']:.2f} ${s['dd']:.2f} | {c['trades']:4d} {c['wr']:.2%} ${c['pnl']:.2f} ${c['dd']:.2f}")
    elif s:
        print(f"{modo:10s} | {s['trades']:4d} {s['wr']:.2%} ${s['pnl']:.2f} ${s['dd']:.2f} | ML no disponible")
