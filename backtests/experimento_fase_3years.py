...
START_DATE = datetime(2023, 4, 1, 0, 0, tzinfo=timezone.utc)
END_DATE   = datetime(2026, 6, 23, 23, 59, tzinfo=timezone.utc)
...
def load_cache():
    data_all = {}
    for sym in SYMBOLS:
        print(f"Cargando {sym} desde caché...")
        sym_data = {}
        for tf in ['1d','4h','1h','15m','5m']:
            ...
            if 'timestamp' not in df.columns:
                print(f"  ⚠ {fname} sin columna timestamp")
                sym_data[tf] = []
                continue
            # Convertir a lista de dicts (formato original)
            klines = []
            ...
            sym_data[tf] = klines
            print(f"  {tf}: {len(klines)} velas en rango")
        data_all[sym] = sym_data
    return data_all

# ──────────────── MOTOR (con phase_mode) ────────────────
    ...
    def run(self, klines, current_price):
        ...
        if signal != 'WAIT':
            if self.phase_mode == 'strict':
                ...
                ci_val = indicators['1h'].get('bb_width', 0.03)
                if phase_h1 in ('ranging', 'neutral') and ci_val is not None and ci_val > 0.03:
                    signal = 'WAIT'
            # 'free' no restringe nada
...
def run_backtest(data_all, phase_mode):
    ...
    for sym in SYMBOLS:
        ...
        engine = ScalpingEngine(sym, SYMBOL_CONFIG[sym], capital=capital,
        ...
        for i in range(10, len(candles_1h)):
            ...
            capital += pnl
            trades_list.append({'pnl_neto': pnl})
    if not trades_list:
        return pd.DataFrame(columns=['pnl_neto']), 0
...
try:
    ...
    ml_disponible = False
    print("⚠ Modelo ML no encontrado, se evaluará sin filtro ML")
...
for mode in modos:
    ...
    df_trades, capital_final = run_backtest(data_all, mode)
    if df_trades.empty:
        resultados_raw[mode] = {'trades':0,'wr':0,'pnl':0,'dd':0}
        resultados_ml[mode] = {'trades':0,'wr':0,'pnl':0,'dd':0}
        continue
    # Guardar CSV
    df_trades.to_csv(f'resultado_{mode}.csv', index=False)
    # Métricas sin ML
    wr = (df_trades['pnl_neto'] > 0).mean()
    pnl_total = df_trades['pnl_neto'].sum()
    curve = (INITIAL_CAPITAL + df_trades['pnl_neto'].cumsum()).values
    peak = np.maximum.accumulate(curve)
    dd = (peak - curve).max()
    resultados_raw[mode] = {'trades': len(df_trades), 'wr': wr, 'pnl': pnl_total, 'dd': dd}
    print(f"   Sin ML: Trades={len(df_trades)}, WR={wr:.2%}, PnL=${pnl_total:.2f}, DD=${dd:.2f}")
    
    if ml_disponible:
        # Preprocesar para ML (necesitamos las features, pero el CSV de trades no las tiene; 
        # para este experimento simplificamos: el motor no guardó features. 
        # Así que no podemos aplicar ML a menos que regeneremos con features.
        # Salteamos el filtro ML por ahora, o decimos que el experimento mide solo fase.
        pass

print("\n" + "="*60)
print("📊 COMPARATIVA FINAL (SIN FILTRO ML)")
print("="*60)
for mode in modos:
    r = resultados_raw[mode]
    print(f"{mode:10s}: Trades={r['trades']:4d} | WR={r['wr']:.2%} | PnL=${r['pnl']:.2f} | DD=${r['dd']:.2f}")
