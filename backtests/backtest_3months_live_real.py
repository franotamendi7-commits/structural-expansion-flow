...
for sym in SYMBOLS:
    ...
    engine = ScalpingEngine(symbol=sym, capital=capital, risk_pct=FIXED_RISK_PCT, debug_filters=False)
    ...
    for i in range(10, len(candles_1h)):
        ...
        if res['setup_state'] == 'EXECUTE' and res.get('trade'):
            ...
            # Buscar el índice en velas de 5m para la simulación de salida
            # Tomamos la vela de 5m que corresponde a este timestamp de 1h
            pnl_neto, events, exit_type = simulate_exit(
                direction, entry_real, sl, tp1, contracts, data['5m'],
                next(idx for idx, c in enumerate(data['5m']) if c['timestamp'] >= ts)
            )
            capital += pnl_neto
            all_trades.append({
                'symbol': sym,
...
if all_trades:
    ...
    csv_name = 'backtest_3months_live_real.csv'
    ...
    print("\n========== RESULTADOS 3 MESES (CONDICIONES REALES) ==========")
